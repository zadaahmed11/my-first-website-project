...
export default function ProductDetails({ productsData }) {
  ...
  if (!product) {
    return <div className="text-center py-20 text-xl font-bold">{t('productNotFound')}</div>;
  }
  ...
  const handlePresetSelect = (fraction, fractionTextKey) => {
    setSelectedPreset({ fraction, fractionText: t(fractionTextKey), key: fractionTextKey });
    setInputPrice('');
    setInputQty('');
  };
  ...
  const handleAddToCartAction = () => {
    ...
    if (selectedPreset) {
      ...
      alert(lang === 'en' ? 'Please select a weight or enter a custom amount first!' : 'الرجاء تحديد وزن أو كتابة قيمة مخصصة أولاً!');
      return;
    }

    addToCart(product, finalUnitVal, finalQtyText, finalPrice);
    alert(lang === 'en' ? 'Product successfully added to your cart!' : 'تم إضافة الوزن المختار للعربة بنجاح!');
  };
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-8 p-6 md:p-10 border border-stone-100">
        ...
        <div>
          <div className="relative rounded-2xl overflow-hidden h-80 bg-stone-50 border">
          ...
          <div className="mt-6">
            <h1 className="text-2xl font-black text-stone-900 mb-2">{cleanProductName(product.name)}</h1>
        ...
        <div className="flex flex-col justify-between bg-stone-50 p-6 rounded-2xl border border-stone-200/60">
          <div>
            <h3 className="text-lg font-black text-stone-800 mb-4 border-b pb-2">{t('weightPriceTitle')}</h3>
            <p className="text-xs text-stone-500 mb-5">
              {t('originalPrice')}: <span className="font-black text-[#0b422a] text-sm">{product.pricePerKg} {t('currency')}</span> / {unitLabel}
            </p>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`py-2.5 rounded-xl text-xs font-black border transition-all ${selectedPreset?.key === item.key ? 'bg-[#0b422a] text-white border-[#0b422a] shadow-xs' : 'bg-white text-stone-700 hover:bg-stone-100'}`}
                  >
                    {item.label}
                  </button>
                ))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="number" value={inputPrice} onChange={(e) => handlePriceChange(e.target.value)} placeholder="EGP" className="w-full p-3 text-sm border rounded-xl bg-white focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label className="block text-[10px] text-stone-400 mb-1">{t('enterQty')} ({unitLabel})</label>
                <input type="number" value={inputQty} onChange={(e) => handleQtyChange(e.target.value)} placeholder="0.00" className="w-full p-3 text-sm border rounded-xl bg-white focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>


            <div className="mt-6 p-4 bg-emerald-50/60 rounded-xl border border-emerald-100/60">
              <p className="text-xs font-medium text-emerald-900">
                {t('currentCalc')}{' '}
                <span className="font-black text-[#0b422a] block text-base mt-1">
                  {selectedPreset 
                    ? `${(product.pricePerKg * selectedPreset.fraction).toFixed(2)} ${t('currency')} [${selectedPreset.fractionText} ${unitLabel}]` 
                    : inputPrice && inputQty 
          ...
          <div className="mt-8 space-y-2.5">
            <button 
              ...
              className="w-full bg-[#0b422a] text-white py-3.5 rounded-xl font-black text-sm shadow hover:bg-emerald-800 transition-all"
            >
              {t('addToCartBtn')}
            </button>
            <div className="grid grid-cols-2 gap-2.5">
              <button onClick={() => navigate('/')} className="bg-stone-200 text-stone-800 py-3 rounded-xl font-bold text-xs hover:bg-stone-300 transition">
                {t('continueShopping')}
              </button>
              <button onClick={() => navigate('/cart')} className="bg-amber-500 text-stone-900 py-3 rounded-xl font-black text-xs hover:bg-amber-400 transition">
                {t('goToCartPage')}
              </button>
            </div>
          </div>
