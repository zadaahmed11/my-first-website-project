      ...
      <div className="container mx-auto flex justify-between items-center gap-2">
        ...
        <div className="flex items-center gap-3 md:gap-6 shrink-0">
          ...
          <button 
            ...
            className="bg-emerald-50 text-emerald-800 border border-emerald-100 px-2.5 py-1 md:px-4 md:py-1.5 rounded-lg text-[11px] md:text-xs font-bold hover:bg-emerald-100 transition-all whitespace-nowrap"
          >
            {lang === 'en' ? 'العربية' : 'English'}
          </button>

          <div 
            className="relative p-2 md:p-2.5 bg-stone-50 hover:bg-emerald-50 rounded-xl cursor-pointer text-stone-700 hover:text-emerald-800 transition-all border border-stone-100/60"
            onClick={() => setIsOpen(!isOpen)}
          >
            <ShoppingCart className="w-4 h-4 md:w-5 md:h-5" />
            {cart.length > 0 && (
              <span className="absolute -top-1 -right-1 md:-top-1.5 md:-right-1.5 bg-amber-500 text-stone-900 text-[9px] md:text-[10px] font-black rounded-full w-3.5 h-3.5 md:w-4 md:h-4 flex items-center justify-center shadow-xs">
                {cart.length}
              </span>
            )}
          </div>
